...
# Initialize buffers
emg_buffers = [[0] * BUFFER_SIZE for _ in range(NUM_CHANNELS)]   #change to  (NUM_CHANNELS) to see all 8 channels)
...
def update_graph(n):
    ...
    traces = [
        go.Scatter(
            x=x_vals,
            y=emg_buffers[i][-length:],  # This is where your error occurred
            mode='lines',
            name=f'Ch {i + 1}'
        ) for i in range(NUM_CHANNELS)
    ]
